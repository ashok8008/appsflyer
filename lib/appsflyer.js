...
export async function importRawReport(db, reportType, fromDate, toDate, { trigger = 'manual' } = {}) {
  ...
  try {
    ...
    for (const raw of rows) {
      ...
      // Fallback: parse Original URL for OneLink suffix and map to publisher by code/name
      let mappedPublisher = publisher
      if (!mappedPublisher && norm.original_url) {
        const m = String(norm.original_url).match(/\/([A-Za-z0-9_]+)\/?(?:\?|#|$)/)
        if (m) {
          const code = m[1].toUpperCase()
          mappedPublisher = await db.collection('publishers').findOne({ public_code: code })
        }
      }
      await db.collection('appsflyer_events').insertOne({
        ...
        raw_row_id: null,
        ...norm,
        publisher_id: mappedPublisher?.id || null,
