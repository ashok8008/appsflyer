...
agent_communication:
  - agent: "main"
    message: "Phase A (Polymarket workflow pivot) complete. Major changes: (1) tracking_mode + direct_polymarket_url on publisher_campaigns - assignment dialog now offers direct/clickvibe/both. (2) New /api/uploads/preview + /api/uploads/import endpoints supporting CSV and multi-sheet XLSX. Auto-detects columns (Promo Code, Signups, Depositors, Traders, Qualified, CPA, Cost, Date, etc.). TOTAL sheets are skipped but their sums captured as upload metadata. (3) New daily_performance collection stores uploaded conversion data. (4) Report aggregator now combines AppsFlyer clicks/installs with uploaded conversions, priority: uploaded > AppsFlyer > clickvibe redirects. (5) New KPI dashboards (admin + publisher) with 7 cards: Clicks, Installs, Signups, Depositors, Traders, Qualified Paid, Cost - plus Deposit Rate, Qual Rate. (6) Date presets 7d/30d/90d/Current/Previous month/All/Custom. (7) Copy-for-Sheets button. (8) AppsFlyer Original URL parsing maps rows to publishers by OneLink suffix. (9) Publisher portal renamed 'My Offers & Links' showing direct Polymarket URL prominently with copy button. Verified end-to-end: created Polymarket campaign + 7 publishers (VICE, POPCULTURE, TFM, SUGGEST, RARE, WIDEOPEN, TFMGIRLS) each with direct OneLink, uploaded promo CSV with 7 rows, all matched, VICE publisher login shows correct numbers (7 signups, 6 depositors, $320 cost). All test/demo data was cleared before this phase."
  - agent: "testing"
