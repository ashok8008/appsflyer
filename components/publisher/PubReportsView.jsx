...
export default function PubReportsView() {
  ...
  useEffect(() => {
    ...
    api(`/publisher/reports/overview?${qs}`).then(setData).catch(() => {})
  }, [dr.from, dr.to, groupBy])

  const t = data?.totals || {}
  const headers = ['key', 'name', 'clicks', 'installs', 'signups', 'depositors', 'traders', 'qualified_paid', 'cost']
  const labels = ['Key', 'Name', 'Clicks', 'Installs', 'Signups', 'Depositors', 'Traders', 'Qualified', 'Payout']
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Reports</h1>
          <p className="text-slate-500">Your performance — filter, copy, export</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => copyForSheets((data?.breakdown || []), headers, labels)}><Copy className="w-4 h-4 mr-1" />Copy for Sheets</Button>
          <Button variant="outline" onClick={() => downloadCsv(`/publisher/reports/export.csv?from=${dr.from}&to=${dr.to}&group_by=${groupBy}`)}><Download className="w-4 h-4 mr-1" />Export CSV</Button>
        </div>
      </div>
      <Card>
        ...
        <CardContent className="p-0 overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b text-slate-600">
              <tr><th className="text-left p-3">{groupBy}</th><th className="text-right">Clicks</th><th className="text-right">Installs</th><th className="text-right">Signups</th><th className="text-right">Depositors</th><th className="text-right">Traders</th><th className="text-right">Qualified</th><th className="text-right p-3">Payout</th></tr>
            </thead>
            <tbody>
              ...
              {(!data?.breakdown || data.breakdown.length === 0) && <tr><td colSpan={8} className="p-8 text-center text-slate-400">No data</td></tr>}
            </tbody>
          </table>
        </CardContent>
      </Card>
