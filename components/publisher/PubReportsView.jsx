    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500">Filter and export your performance</p>
        </div>
        <Button variant="outline" onClick={() => downloadCsv(`/publisher/reports/export.csv?from=${from}&to=${to}&group_by=${groupBy}`)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {[
          ...
          ['Payout', '$' + (t.revenue || 0).toLocaleString()],
        ].map(([l, v]) => (
        ...
        <CardContent className="p-0">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b text-slate-600">
              <tr><th className="text-left p-3">Key</th><th className="text-right">Clicks</th><th className="text-right">Installs</th><th className="text-right">Events</th><th className="text-right">CVR</th><th className="text-right p-3">Payout</th></tr>
            </thead>
            <tbody>
              ...
              {(!data?.breakdown || data.breakdown.length === 0) && <tr><td colSpan={6} className="p-8 text-center text-slate-400">No data</td></tr>}
            </tbody>
          </table>
        </CardContent>
      </Card>
